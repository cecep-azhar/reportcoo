         ...
         row.RelativeItem().AlignMiddle().Column(textColumn =>
        {
             ...
             ? _placeholderResolver.Resolve(line.PlaceholderKey, data) ?? line.Text
                   : line.Text;
                    ...
                    var headerText = text;
                    textColumn.Item().PaddingVertical(line.MarginTop, Unit.Millimetre).AlignCenter().Text(txt =>
                  {
                          txt.DefaultTextStyle(style =>
                          {
        ...
        container.Padding(layout.Padding).Table(table =>
        {
            ...
            if (layout.ColumnsCount == 2)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    if (leftField != null)
                    {
                        table.Cell().Text(txt =>
                        {
                            txt.DefaultTextStyle(s => leftField.LabelBold ? s.Bold() : s);
                            txt.Span(leftField.Label);
                        });
                        table.Cell().Text(leftField.Separator);
                        var leftValue = _placeholderResolver.Resolve(leftField.PlaceholderKey, data) ?? "";
                        table.Cell().Text(txt =>
                        {
                            txt.DefaultTextStyle(s => leftField.ValueBold ? s.Bold() : s);
                            txt.Span(leftValue);
                        });
                    }
                    else
                    ...
                    if (rightField != null)
                    {
                        table.Cell().Text(txt =>
                        {
                            txt.DefaultTextStyle(s => rightField.LabelBold ? s.Bold() : s);
                            txt.Span(rightField.Label);
                        });
                        table.Cell().Text(rightField.Separator);
                        var rightValue = _placeholderResolver.Resolve(rightField.PlaceholderKey, data) ?? "";
                        table.Cell().Text(txt =>
                        {
                            txt.DefaultTextStyle(s => rightField.ValueBold ? s.Bold() : s);
                            txt.Span(rightValue);
                        });
                    }
                    else
            ...
            else // Single column
            {
                ...
                foreach (var field in visibleFields)
                {
                    table.Cell().Text(txt =>
                    {
                        txt.DefaultTextStyle(s => field.LabelBold ? s.Bold() : s);
                        txt.Span(field.Label);
                    });
                    table.Cell().Text(field.Separator);
                    var value = _placeholderResolver.Resolve(field.PlaceholderKey, data) ?? "";
                    table.Cell().Text(txt =>
                    {
                        txt.DefaultTextStyle(s => field.ValueBold ? s.Bold() : s);
                        txt.Span(value);
                    });
                }
            }
    ...
    private void ComposeResultSection(IContainer container, ResultSection result, ReportData data)
    {
        ...
        container.Padding(result.Padding).MinHeight(result.MinHeight).Column(column =>
        {
            ...
            column.Item().PaddingTop(5).Text(txt =>
            {
                ...
                txt.Span(string.IsNullOrEmpty(resultText) ? "-" : resultText);
            });
        });
    }
